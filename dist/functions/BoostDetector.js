...
const BoostDetector = (oldState, newState) => {
  ...
  const boostChannel = oldState?.guild.channels.cache.get(
    _static.default.boostChannelId
  );
  ...
  if (!oldStatus && newStatus) {
    const embed = new _discord.MessageEmbed()
      ...
      .setDescription(`${newState} Boosted The Server!!!`)
      .addField(
        "Total Boosts",
        `${newState?.guild?.premiumSubscriptionCount}`,
        false
      );
    boostChannel.send({
      embeds: [embed],
  ...
  if (!newStatus && oldStatus) {
    const embed = new _discord.MessageEmbed()
      ...
      .setDescription(`${newState} Unboosted The Server!!!`)
      .addField(
        "Total Boosts",
        `${newState?.guild?.premiumSubscriptionCount}`,
        false
